...
def create_symmetry_matrix_pca(input_file,output_file,no_comp):
	dataset = pd.read_csv(input_file,sep=",",header=None)
	csv_list = dataset.iloc[0:,0:].values
	...
	with open(output_file,'w') as outputcsv_file:
		...
		for i in range(0,row):
			...
			print len(col1)
			spamwriter.writerow(col1)
# create_symmetry_matrix('wine_randomized.txt','wine_dist_rand.txt')
# create_symmetry_matrix_pca('wine_randomized.txt','wine_dist_rand_pca.txt',5)
create_symmetry_matrix('iris_randomized.txt','iris_dist_rand.txt')
create_symmetry_matrix_pca('iris_randomized.txt','iris_dist_rand_pca.txt',2)
