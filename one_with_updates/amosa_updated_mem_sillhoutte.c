...
int arcsize;
double Temperature, TMin = 0.001, TMax = 100, alpha = 0.75;
int  MaxLen, MinLen;
double **area;
int POP_SIZE;
char file[30];
...
double ComputeFitnessSillhoutte(struct archive_element *c,int view,int no_points);
int doc_count;
...
printf("\n6\n");
				...
				for(k=0;k<total_index;k++)
				{
				    if(no_view>1)
				    {
						if (FitnessIndex[k] == 1) Obj[k] = ComputeFitnessSillhoutte(&new_pool,1,n);
						else if (FitnessIndex[k] == 2) Obj[k] = ComputeFitnessSillhoutte(&new_pool,2,n);
				        else if (FitnessIndex[k] == 3) Obj[k] = ComputeFitnessDAI(&new_pool);
				    }
				    else
				    {
				    	if (FitnessIndex[k] == 1) Obj[k] = ComputeFitnessXB(&new_pool,0);
						else if (FitnessIndex[k] == 2) Obj[k] = ComputeFitnessSillhoutte(&new_pool,0,n);
				    }
			    }
...
void ComputeMembership(struct archive_element *Chrom,int view)
{
     ...
     for(Index1=0;Index1<n;Index1++)
    {
    	min1=9999999.99;
       flag=0;
        for(Index2=0;((Index2<MaxLen)&&(flag==0));Index2++)
       {
          if(Chrom->z[Index2].x[0]!=DUMMY) // then it is a gene center
          {
            i=(int)Chrom->z[Index2].x[0]; //index of gene center
            j=(int)points[Index1][0];    // index of gene/ chromosome
              if(view==0 || view==2)
              ...
              if(view==1)
                ...
                } // loop for gene center that has value greater than dummy
                if(flag==1)
                break;
            } //loop for gene center mavlen
	...
	printf("\n++++chrom->len=%d+++++\n",Chrom->len);
	fflush(stdout);
	printf("\n checking if membership worked fine \n");
	int count_mem=0;
     for ( Index2 = 0; Index2 < MaxLen;Index2++)
		{
			if(Chrom->z[Index2].x[0]!=DUMMY && Chrom->z[Index2].x[0]>=0.0)
				{
				  for(Index1=0;Index1<n;Index1++)
				  {
				  	if(Chrom->Membership[view][Index2][Index1]==1.0)
				  		count_mem++;
				  }
				 }
		}
...
void form_nondominated_archive()
{
	...
	for(i=0;i<POP_SIZE;i++)
	{
		...
		for(j=0;j<total_index;j++)
		{
			if(no_view>1)
			{
				if (FitnessIndex[j] == 1) Obj1 = ComputeFitnessSillhoutte(&pool[i],1,n);
				else if (FitnessIndex[j] == 2) Obj1 = ComputeFitnessSillhoutte(&pool[i],2,n);
				else if (FitnessIndex[j] == 3) Obj1 = ComputeFitnessDAI(&pool[i]);
			}
			else
			{
				if (FitnessIndex[j] == 1) Obj1 = ComputeFitnessSillhoutte(&pool[i],0,n);
				else if (FitnessIndex[j] == 2) Obj1 = ComputeFitnessSillhoutte(&pool[i],0,n);
...
printf("\n03\n");
		    ...
		    for(j=0;j<total_index;j++)
		    {
				if(no_view>1)
				{
			        if (FitnessIndex[j] == 1) Obj1 = ComputeFitnessSillhoutte(&new_pool,1,n);
					else if (FitnessIndex[j] == 2) Obj1 = ComputeFitnessSillhoutte(&new_pool,2,n);
					else if (FitnessIndex[j] == 3) Obj1 = ComputeFitnessDAI(&new_pool);
				}
				else
				{
					if (FitnessIndex[j] == 1) Obj1 = ComputeFitnessXB(&new_pool,0);
					else if (FitnessIndex[j] == 2) Obj1 = ComputeFitnessSillhoutte(&new_pool,0,n);
				}
...
double ComputeFitnessSillhoutte(struct archive_element *c,int view,int no_points)
{
	int i,j,k,l,count_inclus=0,count_outclus=0,len=0,count;
	double min=(double)LONG_MAX,sum=0.0,b_i=0.0,a_i=0.0,avg=0.0, s_i=0.0, s_k=0.0, SI=0.0,Di3;
	printf("\n in SI\n");
	int total_count=0;
	for(i=0;i<MaxLen;i++)
	{ 
		...
		{ 
			if(c->Membership[view][i][j]==1)
				count++;
		
		}
		total_count=total_count+count;
		printf("ith center:%d len=%d",i,count);
		//getchar();
		printf("\n");
		   ...
		   for(j=0;j<no_points;j++)
			{
				...
				if(c->Membership[view][i][j]==1.0)
				{
					...
					{
						if(c->Membership[view][i][k]==1.0)
						{
							if(view==0 || view==2)
					    Di3 =  distance_matrix[1][j][k]; //FindDistance(Chrom->z[Index2].x,points[Index1],view);
                       if(view==1)
                	    Di3 =  distance_matrix[0][j][k];
                	    sum+=Di3;
						}
					...
					for(k=0;k<MaxLen;k++) // Measuring inter cluster distance
					{
						if( c->z[k].x[0]<DUMMY)
					    {
					    count_outclus=0;
						...
						if(i!=k)
						{
							...
							for(l=0;l<no_points;l++) //counting total number of points in the cluster C_k where C_k != C_i
							{
								if(c->Membership[view][k][l]==1.0)
								{
									...
									if(view==0 || view==2)
					                Di3 =  distance_matrix[1][l][j]; //FindDistance(Chrom->z[Index2].x,points[Index1],view);
                                    if(view==1)
